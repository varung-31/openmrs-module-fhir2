...
public class PatientFhirResourceProvider implements IResourceProvider {
	...
	public Bundle searchPatients(@OptionalParam(name = Patient.SP_NAME) StringOrListParam name,
			...
			@Sort SortSpec sort) {
		return FhirUtils.convertSearchResultsToBundle(patientService
				.searchForPatients(name, given, family, identifier, gender, birthDate, deathDate, deceased, city, state,
						postalCode, sort));
	}
